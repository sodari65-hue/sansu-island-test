...
export const FIELD = { width: 72, height: 44, spawn: { x: 12.5, y: 28.5 } };
...
export const HOUSES = [{ x: 8, y: 23, w: 4, h: 3 }, { x: 16, y: 24, w: 4, h: 3 }, { x: 9, y: 31, w: 4, h: 3 }];
export const FOES = [
...
export function tile(map, x, y, doors = 0, grade = 5){
  ...
  if (onPath(x, y) || (x >= 7 && x <= 21 && y >= 27 && y <= 30)) return 'path';
...
export function leaveCave(state){ rememberDungeon(state); const spot=DUNGEON_SPOTS.find(p=>p.domain===state.activeDungeon)||ENTRANCE;state.map = 'field'; state.x = spot.x; state.y = spot.y + 2; }
...
export function returnVillage(state){ state.map = 'field'; Object.assign(state, FIELD.spawn); }
...
export function freshState(name, grade = 5){
  ...
  return { version: 1, name, grade, regionGrade:grade, activeDungeon:'A', dungeonProgress:{}, defeatedBosses:[], clearedQuests:[], map: 'field', ...FIELD.spawn, facing: 1, doors: 0, cleared: false,
...
function validSave(s, name){
  ...
  if (!Number.isInteger(s.grade) || s.grade < 1 || s.grade > 6 || !['field', 'cave'].includes(s.map)) return false;
  ...
  if (!['lantern', 'wand'].includes(s.item) || !/^#[\da-f]{6}$/i.test(s.hat) || typeof s.ruby !== 'boolean') return false;
...
export function loadState(storage, trial, name){
  ...
  if (!canStand(s.map, s.x, s.y, s.doors, s.regionGrade)){
    ...
    Object.assign(s, besideChest ? {x:TREASURE.x,y:TREASURE.y+1} : s.map === 'cave' ? CAVE.spawn : FIELD.spawn);
  }
  return s;
}
